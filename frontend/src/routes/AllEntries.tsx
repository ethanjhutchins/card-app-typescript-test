...
export default function AllEntries() {
  ...
  if (entries.length == 0) {
    return (
      <section>
        <h1 className={`text-center font-semibold text-2xl m-5 ${darkMode ? "text-gray-50" : "text-black"}`}>
          You don't have any card
        </h1>
        <p className={`text-center font-medium text-md ${darkMode ? "text-gray-50" : "text-black"}`}>
          Lets{" "}
      ...
      {entries.map((entry: Entry, index: number) => {
        return (
          <div
            id={entry.id}
            key={index}
            className={`shadow-md m-3 p-4 rounded flex flex-col justify-between ${
